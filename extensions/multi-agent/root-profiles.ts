...
function serializeProfile(input: SaveRootProfileInput): string {
  const skills = Array.from(new Set((input.skills || []).map(String).map((s) => s.trim()).filter(Boolean)));
  const skillTemplates = Array.from(new Set((input.skillTemplates || []).map(String).map((s) => s.trim()).filter(Boolean)));
  const frontmatterLines = [
    ...
    skillTemplates.length ? `skillTemplates: ${skillTemplates.join(", ")}` : undefined,
...
function profileDetail(profile: RootOrchestratorProfile): RootOrchestratorProfileDetail | undefined {
  try {
...
export function getRootProfileDetail(name: string, cwd: string): RootOrchestratorProfileDetail | undefined {
  const profile = getRootProfile(name, cwd);
  return profile ? profileDetail(profile) : undefined;
}

export function saveRootProfile(input: SaveRootProfileInput, cwd: string): { success: boolean; path?: string; detail?: RootOrchestratorProfileDetail; error?: string; status?: number } {
  const name = (input.name || "").trim();
  if (!name) return { success: false, error: "name is required", status: 400 };
  if (!safeProfileName(name)) return { success: false, error: "name may only contain letters, numbers, dot, underscore, and dash", status: 400 };
  ...
  if (existing?.readOnly) return { success: false, error: "root orchestrator profile is read-only", status: 403 };

  let filePath: string;
  if (existing) {
    const detail = profileDetail(existing);
    if (input.expectedHash && detail?.hash !== input.expectedHash) return { success: false, error: "profile changed on disk; reload before saving", status: 409 };
  ...
  try {
    ...
    return { success: true, path: filePath, detail: getRootProfileDetail(name, cwd) };
  } catch (err: any) {
    return { success: false, error: err.message, status: 500 };
  }
}

export function copyRootProfile(sourceName: string, input: Omit<SaveRootProfileInput, "instructions" | "skills" | "skillTemplates">, cwd: string): { success: boolean; path?: string; detail?: RootOrchestratorProfileDetail; error?: string; status?: number } {
  const source = getRootProfileDetail(sourceName, cwd);
  if (!source) return { success: false, error: "profile not found", status: 404 };
  if (getRootProfile(input.name, cwd)) return { success: false, error: `profile '${input.name}' already exists`, status: 409 };
...
export function deleteRootProfile(name: string, cwd: string): { success: boolean; error?: string; status?: number } {
  if (!safeProfileName(name)) return { success: false, error: "invalid profile name", status: 400 };
  const profile = getRootProfile(name, cwd);
  if (!profile) return { success: false, error: "profile not found", status: 404 };
  if (profile.readOnly) return { success: false, error: "root orchestrator profile is read-only", status: 403 };
  try {
