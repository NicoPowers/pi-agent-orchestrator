...
import type { OrchestratorLibrariesInfo, SkillDetailInfo, SkillInfo } from "../../types.js";
...
export function SkillLibraryPanel({ skills, diagnostics, skillTemplates, onEditTemplate, onChanged }: { skills: SkillInfo[]; diagnostics: SkillDiagnostic[]; skillTemplates: TemplateInfo[]; onEditTemplate: (template: TemplateInfo) => void; onChanged: () => void }) {
  ...
  const addToTemplate = async () => {
    ...
    const res = await fetch("/api/skill-templates", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: template.name, description: template.description, audience: templateAudience(template), autoApply: templateAutoApply(template), skills }) });
